...
string MP2Node::readKey(string key, int transID) {
	...
	if(result.length() > 0)
		log->logReadSuccess(&memberNode->addr, coordinator, transID, key, result);

	else
	{
		log->logReadFail(&memberNode->addr, coordinator, transID, key);
		cout << "read fail at transID " << transID << " for key " << key << endl;
	}
	...
	while ( !memberNode->mp2q.empty() ) {
		...
		switch(msgType)
    	{
			...
			case(READ):
				...
				cout << "read result is " << readResult << endl;

				if(readResult.length() > 0)
				{
					requestSucessfull = true;
					replyRead = readResult;
				}
				else
					requestSucessfull = false;						
			...
			if(qCount.length() > 0)
			{
				...
				if(transID > 4000 && transID < 6001)		// read message
				{
					replyStatus = checkCreateReply(transID, qCount, replyKey, replyData, msgSuccessful);
					if(replyStatus == 1)
					{
						log->logReadSuccess(&memberNode->addr, coordinator, transID, replyKey, replyRead);
						posQuorum->deleteKey(sTransID);	
					}
					if(replyStatus == 3)
					{
						log->logReadFail(&memberNode->addr, coordinator, transID, replyKey);
						posQuorum->deleteKey(sTransID);	
					}
				}
