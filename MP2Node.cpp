...
//		cout << "size of ring is " << ringSize << endl;
	}
...
//	cout << "client read request " << newReadMsg->toString() << endl;
	// key value
	string delim = " ";
	...
	while ( !memberNode->mp2q.empty() ) {
		...
		switch(msgType)
    	{
			...
			case(READ):
			{
				...
				if(readResult.length() > 0)		// issue here. 
				{
...
//				}
			...
			case(REPLY):	// used for create / delete / update
			{
				coordinator = true;
				string transID_Data;
...
void MP2Node::stabilizationProtocol() 
{
	...
	if(firstExpectedAddress != firstActualAddress)				// first neighbor has failed
	{
		if(firstActualAddress == secondExpectedAddress)			// second neighbor shifted into first neighbor
		{
			...
			// need to copy everything in my HT into ring 2 position further than mine
			Address* sendToAddress = actualSecondNeighbor.getAddress();
			hasMyReplicas.clear();
			...
			// ******* need to send msg to other node for each item in HT ********
